...
//-------------------- Service order Summary details  -------------------//
    ...
    function service_order_summary($user_master_id,$service_order_id){
      ...
      // WHERE so.id='$service_order_id' AND so.customer_id='$user_master_id'";
        ...
        $service_query="SELECT DATE_FORMAT(so.start_datetime,'%Y-%m-%d %r') as start_time,DATE_FORMAT(so.finish_datetime,'%Y-%m-%d %r') as finish_time,IFNULL(lu.phone_no,'') as phone_no,IFNULL(spp.full_name,'') AS full_name,IFNULL(spd.owner_full_name,'') AS owner_full_name,
